...
{
    public class ReactQueueListener : IQueueListener<ReactEvent>
    {
        ...
        public async Task ListenAsync(CancellationToken _cancellationToken)
        {
            ...
            consumer.ReceivedAsync += async (model, args) =>
            {
                ...
                var reactEvent = JsonSerializer.Deserialize<ReactEvent>(messageJson);

                if (reactEvent != null)
                {
                    if (reactEvent.EventType == ReactEventType.Like)
                    {
                        await _feedCommandService.IncrementReactsCountAsync(reactEvent);
                    }
                    else if (reactEvent.EventType == ReactEventType.Unlike)
                    {
                        await _feedCommandService.DecrementReactsCountAsync(reactEvent);
                    }
                }
            };
